...
--------------------------------------------------------
--------------------------------------------------------
--  DDL for Package Body XXVEN_AR_CUPOM_ANALISA_PK2
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XXVEN_AR_CUPOM_ANALISA_PK2" as
  -- $Header: XXVEN_AR_CUPOM_ANALISA_PK2.pkb 120.2 2019/01/30 00:00:00 appldev noship $
  -- +=================================================================+
      ...
      if v_sequencial != r_cupom.sequencial then
        ...
        if v_qtd_pagamento != 0 then
         ...
         begin
           ...
           --                                             , envio_data_hora = sysdate
           --                                             , envio_erro = 'NAO ENCONTRADO INFORMACOES NA TABELA DE PAGAMENTO: CUPOM: ' || r_cupom.cupom_venda ||
           --                                                                                                                'Loja: ' || r_cupom.codigo_loja_pdv ||
           --                                                                                                                 'PDV: ' || r_cupom.caixa
           -- where id_sequencial = r_cupom.id_sequencial;
           ...
           --fnd_file.put_line(fnd_file.log,'NAO ENCONTRADO INFORMACOES NA TABELA DE PAGAMENTO: CUPOM: ' || r_cupom.cupom_venda ||
           --                                                                                   'Loja: ' || r_cupom.codigo_loja_pdv ||
           --                                                                                    'PDV: ' || r_cupom.caixa);

          -- dbms_output.put_line('NAO ENCONTRADO INFORMACOES NA TABELA DE PAGAMENTO: CUPOM: ' || r_cupom.cupom_venda ||
          --                                                                          'Loja: ' || r_cupom.codigo_loja_pdv ||
          --                                                                           'PDV: ' || r_cupom.caixa);
            NULL;
         end;
  ...
  begin
    ...
    for r_pagam in (select distinct
      ...
      begin
        ...
        ----------fnd_file.put_line(fnd_file.log,'Tipo Pagamento encontrado term_id: ' || l_term_id);
        ----------dbms_output.put_line('Tipo Pagamento encontrado term_id: ' || l_term_id);
      exception
        when no_data_found then
          update tb_anali_ebs_ped_venda_pagam@intprd set envio_status    = '30'
           ...
           update tb_anali_ebs_ped_venda_lin@intprd set envio_status    = '30'
                                                       , envio_data_hora = sysdate
                                                       --, envio_erro = 'NAO ENCONTRADO INFORMACOES DE PAGAMENTO: ' ||
                                                       --                         R_PED_PAG_ANALI.codigo_modalidade || '.' || -- 401
                                                       --                         R_PED_PAG_ANALI.instituicao       || '.' || -- MASTERCARD
                                                       --                         R_PED_PAG_ANALI.NUMERO_PARCELA
            where id_ped_venda_cab = p_sequencial
              and envio_status is null;
  ...
  begin
    ...
    for r_item in (select msi.inventory_item_id item_id, msi.segment1, msi.global_attribute3 origem, msi.description item_descricao, msi.primary_uom_code uom_code
                    ...
                    where msi.segment1 = '67247'
                      ...
                      --and pag.codigo_modalidade = '302'
                  /*
                   select lin.codigo_filial, lin.estacao, lin.cupom_venda, lin.id_sequencial sequencial, lin.organizacao_venda, lin.quantidade, lin.unidade_medida, lin.valor_item, lin.codigo_item, lin.cfop
                        , lin.idpbms, lin.codigo_pbms, lin.valor_desconto, lin.data_hora
                        , nvl(lin.icms,0) icms, nvl(lin.icms_aliquota,0) icms_aliquota, nvl(lin.pis,0) pis, nvl(lin.pis_aliquota,0) pis_aliquota, nvl(lin.cofins,0) cofins, nvl(lin.cofins_aliquota,0) cofins_aliquota-- linha imposto
                        , msi.inventory_item_id item_id, msi.segment1, msi.global_attribute3 origem, msi.description item_descricao, msi.primary_uom_code uom_code
                        , msi.global_attribute2, msi.global_attribute4 tipo_fiscal, msi.global_attribute5 sit_federal, msi.global_attribute6 sit_estadual
                        , mp.cost_of_sales_account cost_sales
                     ...
                     from tb_anali_ebs_ped_venda_lin@intprd lin
                        , mtl_system_items_b                msi
                        , mtl_parameters                    mp
                    where lin.codigo_item      = msi.segment1
                      and lin.id_ped_venda_cab = p_sequencial
